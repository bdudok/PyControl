...
class Run_task_tab(QtWidgets.QWidget):
    ...
    def man_reward(self):
        if self.board.framework_running:
            self.board.send_reward_msg_to_pyboard('manual_reward')

    def man_open(self):
        if self.board.framework_running:
            self.board.send_reward_msg_to_pyboard('manual_open')

    def man_toggle(self):
        if self.board.framework_running:
            self.board.send_reward_msg_to_pyboard('manual_toggle')

    def man_stim(self):
        if self.board.framework_running:
            self.board.send_reward_msg_to_pyboard('manual_stim')
