...
def lap_reset(force=False):
    ...
    if force:
        #time has passed since the pos was reached, start from >0 pos
        belt_pos.position = belt_pos.position - v.force_lap_reset
    else:
...
def searching(event):
    if event == 'entry':
        ...
        elif belt_pos.position > v.force_lap_reset:
            lap_reset()
        else:
            set_timer('poll_timer', v.poll_resolution, output_event=True)
